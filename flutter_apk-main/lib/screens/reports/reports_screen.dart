  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: loading ? const Center(child: CircularProgressIndicator()) : ListView.builder(itemCount: reports.length, itemBuilder: (_,i){
        final r = reports[i];
        final url = r['report_url'] ?? '';
        return ListTile(title: Text(r['title'] ?? 'Report'), trailing: IconButton(icon: const Icon(Icons.open_in_new), onPressed: () async { if (await canLaunch(url)) await launch(url); }));
      }),
    );
  }
}
